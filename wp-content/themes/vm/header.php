    ...
    <header class="header">
        <div class="header__container">
            <a href="http://le-vieux-moulin.test/" aria-label="Accueil - Le Vieux Moulin">
                <!-- logo -->
                <svg class="house" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="30" height="30" aria-hidden="true">
            ...
            <input type="checkbox" id="menu-toggle" class="header__toggle" aria-label="Ouvrir le menu principal"/>
                        ...
                        <?php foreach ($parents as $group): ?>
                            <div class="header__item">
                                <?php if (!empty($group['children'])): ?>
                                    ...
                                    <label for="submenu-<?= $group['parent']->ID ?>" class="header__link header__link--has-submenu">
                                        <?= esc_html($group['parent']->title) ?> <span class="header__icon-toggle">+</span>
                                    </label>

                                    <div class="header__submenu" id="submenu-panel-<?= $group['parent']->ID ?>" role="menu">
                                        <?php foreach ($group['children'] as $child): ?>
                                            <a href="<?= esc_url($child->url) ?>" role="menuitem" class="header__sublink">
